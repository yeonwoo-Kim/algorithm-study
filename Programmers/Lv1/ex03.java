...
public class ex03 {
    ...
    static String solution(int[] food) {
        // 맨 처음 Queue와 Stack으로 하나씩 빼서 문자열에 저장하는 것을 생각했으나, 문자열 연산으로 처리할 수 있을 것 같아서 계획 변경

        String answer = "";

        // index 0은 물의 개수로, index 1부터 반복 시작
        for (int i = 1; i < food.length; i++) {
            // food의 개수가 각 선수에게 나눌 수 있는 개수가 아니라면 배치하지 않음
           if (food[i] / 2 != 0) {
                String s = String.valueOf(i);
                // answer에 food[i] / 2만큼 숫자 반복해서 배치
                answer += s.repeat(food[i] / 2);
            }
        }
        // StringBuffer로 answer의 문자열을 반대로 바꾸어 붙임
        StringBuilder sb = new StringBuilder(answer);
        /*
            첫번째 선수 음식 배치 | 물 | 두번째 선수 음식 배치
            *******************  0   *******************
         */
